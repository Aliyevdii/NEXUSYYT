...
async def skip(client, m: Message):
    ...
    if len(m.command) < 2:
        ...
        if op == 0:
            ...
            await m.reply("✅ __Növbələr__ **is empty.**\n\n**• userbot səsli söhbəti tərk edir**")
        elif op == 2:
            await m.reply("🗑️ **Növbələrin Təmizlənməsi**\n\n**• userbot səsli söhbəti tərk edir**")
        else:
            await m.reply_photo(
                photo=f"{IMG_3}",
                caption=f"⏭ **Növbəti treki keçildi.**\n\n🏷 **ad:** [{op[0]}]({op[1]})\n💭 **Söhbət:** `{chat_id}`\n💡 **Status:** `Oynamaq`\n🎧 **tərəfindən sorğu:** {m.from_user.mention()}",
...
async def resume(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            await m.reply(
                "▶️ **Trek davam etdi.**\n\n• **Yayımı dayandırmaq üçün istifadə edin**\n» /pause command."
            )
        except Exception as e:
            await m.reply(f"🚫 **Xəta:**\n\n`{e}`")
    else:
...
async def unmute(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            await m.reply(
                "🔊 **Userbot səsi açıldı.**\n\n• **İstifadəçi robotunun səsini söndürmək üçün istifadə edin**\n» /mute command."
            )
        except Exception as e:
            await m.reply(f"🚫 **Xəta:**\n\n`{e}`")
    else:
...
async def cbpause(_, query: CallbackQuery):
    ...
    if not a.can_manage_voice_chats:
        return await query.answer("Yetkiniz Yoxdur😒", show_alert=True)
    chat_id = query.message.chat.id
    if chat_id in QUEUE:
        try:
            await call_py.pause_stream(chat_id)
            await query.edit_message_text(
                "⏯️ yayım dayandırıldı", reply_markup=bttn
            )
        except Exception as e:
...
async def change_volume(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            await m.reply(
                f"✅ **səs səviyyəsi təyin olundu** `{range}`%"
            )
        except Exception as e:
            await m.reply(f"🚫 **xəta:**\n\n`{e}`")
    else:
